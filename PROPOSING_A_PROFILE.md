...
When the profile itself defines one required value, declare it once in
`fixed`. The key is the field path and the field must also be `required`:
...
Use a fixed value only when every port using the profile must declare that
same value. Do not use it for an example, a default or a preference.
