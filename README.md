...
- Protein Sequence
- Protein Structure
- Molecular SMILES (one molecule per port value)

New profiles are added only when they support a real model-to-model connection.
...
A profile can also fix a required field to one value. For example, the
Molecular SMILES profile fixes `representation.structure_format` to `smiles`.
The generated schema then rejects any other value for that profile.
