...
5. Set the new version in `package.json`, `package-lock.json`, `pyproject.toml`,
   ...
   CHANGELOG entry.
6. For a stable release, also check `ga_ready: true` in the generated bundle.
   Do not override this gate.
7. Tag the commit with the version (for example `v0.1.0-alpha.2`) and create the
   GitHub release.
8. Publish the npm and Python packages from that tag.
9. Publish the spec files at their biosimulant.com URLs, and check their bytes
   against the bundle manifest.
10. Update pins in other repositories only after all of the above match.
