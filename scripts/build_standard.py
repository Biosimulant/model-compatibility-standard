...
SET_LIKE_PATHS = [
    ...
    "/contract/representation/syntax_features",
...
def load_profiles() -> list[dict[str, Any]]:
    ...
    for path in sorted(PROFILES_SOURCE.rglob("*.yaml")):
        ...
        fixed_source = source.get("fixed") or {}
        if not isinstance(fixed_source, dict):
            raise SystemExit(f"{path.relative_to(ROOT)}: fixed must be a mapping of field paths to values")
        fixed_values: dict[str, Any] = {}
        for fixed_path, fixed_value in fixed_source.items():
            if not isinstance(fixed_path, str) or not fixed_path:
                raise SystemExit(f"{path.relative_to(ROOT)}: fixed field paths must be non-empty strings")
            if fixed_path == "semantic.concept":
                raise SystemExit(
                    f"{path.relative_to(ROOT)}: semantic.concept is derived from the profile path and must not be fixed manually"
                )
            if fixed_path not in FIELD_INDEX:
                raise SystemExit(f"{path.relative_to(ROOT)}: unknown fixed field: {fixed_path}")
            if fixed_path not in required:
                raise SystemExit(f"{path.relative_to(ROOT)}: fixed field {fixed_path} must also be required")
            schema = (FIELD_INDEX[fixed_path].get("json_schema") or FIELD_INDEX[fixed_path].get("schema") or {})
            errors = sorted(
                Draft202012Validator(schema, format_checker=FormatChecker()).iter_errors(fixed_value),
                key=lambda error: list(error.path),
            )
            if errors:
                raise SystemExit(
                    f"{path.relative_to(ROOT)}: invalid fixed value for {fixed_path}: {errors[0].message}"
                )
            fixed_values[fixed_path] = fixed_value
...
def profile_fixed(profile: dict[str, Any]) -> dict[str, Any]:
    fixed: dict[str, Any] = {}
    for path, value in profile.get("fixed_values", {}).items():
        set_path(fixed, path, value)
