...
const TOKEN = /(\()|(\))|([./])|(\{[^}]*\})|(\d+)(?=$|[./(){}]|\{)|(\[[^\]]*\]|[^./(){}\s]+?)([+-]?\d+)?(?=$|[./(){}]|\{)/y;
export function parseUnit(expression, table) {
    if (typeof expression !== "string" || !expression.trim())
        throw new UnitError("empty unit");
    ...
    const next = () => {
        ...
        if (!match)
            throw new UnitError(`cannot parse ${expression}`);
    ...
    const resolve = (code) => {
        ...
        if (!entry)
            throw new UnitError(`unknown unit code ${code}`);
        used.add(code);
        if (entry.arbitrary)
            arbitrary.add(code);
        if (entry.offset)
    ...
    const component = () => {
        ...
        if (match[1]) {
            ...
            const closing = next();
            if (!closing[2])
                throw new UnitError(`unbalanced parentheses in ${expression}`);
            return inner;
        }
        if (match[4])
            return [1, zero()];
        if (match[3] || match[2])
            throw new UnitError(`unexpected operator in ${expression}`);
        // A bare integer is a factor in its own right: 86400 is not 8 to the 6400th.
        if (match[5])
            return [Number(match[5]), zero()];
    ...
    if (position !== text.length)
        throw new UnitError(`trailing input in ${expression}`);
 ...
 */
export function convertUnit(source, target, table) {
    if (source === target)
        return { factor: 1, offset: 0, affine: false, lossless: true };
    ...
    if (JSON.stringify(left.dimension) !== JSON.stringify(right.dimension))
        return null;
